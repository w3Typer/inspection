...
&nbsp;- номер на договор: <b>${jalbDogNo}</b>`;
  ...
  if (jalbTel !== "") jalbDanni1 = "тел. " + jalbTel;

  if (jalbTel !== "" && jalbKlNo !== "") jalbDanni2 = ", кл. номер " + jalbKlNo;
  else if (jalbKlNo !== "") jalbDanni2 = "кл. номер " + jalbKlNo;

  if ((jalbTel !== "" || jalbKlNo !== "") && jalbDogNo !== "")
    jalbDanni3 = ", номер на договор " + jalbDogNo;
  else if (jalbDogNo !== "") jalbDanni3 = "номер на договор " + jalbDogNo;

  if (jalbTel === "" && jalbKlNo === "" && jalbDogNo === "")
    jalbDanni1 = "!!! ЗА ОПЕРАТОРА СА НЕОБХОДИМИ ПОВЕЧЕ ДАННИ !!!";

  if (provVid1 !== "нормална") provVid11 = provVid1 + " ";
...
Във връзка с подадения от Вас сигнал с вх. № ${signalNo}/${signalData} г., в рамките на правомощията за осъществяване на регулаторни и контролни функции по спазването на нормативната уредба, за която Комисията за регулиране на съобщенията (КРС) отговаря, ще бъде извършена проверка. В интерес на нейната пълнота и качество, моля във възможния за Вас кратък срок да предоставите по електронен път (за предпочитане с електронно писмо, а при невъзможност - по телефон чрез приложение „Viber“) писмена информация в отговор на всеки от поставените въпроси и копия/снимки на документи:`;
  ...
  if (druj1EN !== "A1" && druj1EN !== "Vivacom" && druj1EN !== "Yettel") {
    druj1BG = "ДРУЖЕСТВО_1";
    druj1BGreg = "?????";
    eikDr1 = "?????";
  }
  if (druj2EN !== "A1" && druj2EN !== "Vivacom" && druj2EN !== "Yettel") {
    druj2BG = "ДРУЖЕСТВО_2";
    druj2BGreg = "?????";
    eikDr2 = "?????";
  }
  if (druj3EN !== "A1" && druj3EN !== "Vivacom" && druj3EN !== "Yettel") {
    druj3BG = "ДРУЖЕСТВО_3";
    druj3BGreg = "?????";
    eikDr3 = "?????";
  }
  ...
  document.getElementById("7-7").innerHTML =
    `${signalNo}/${signalData}&#13;&#10;${pismoNo}/${pismoData}`;
  ...
  document.getElementById("8-7").innerHTML =
    `${signalNo}/${signalData}&#13;&#10;${pismoNo}/${pismoData}`;
...
<b>Приложение:</b> КП № С-xxx/${provSrok} г. в електронен вид в „АИДА“.`;
  ...
  document.getElementById("ime-na-faila").innerHTML =
    `__${pismoNoBukviEN}-${pismoNoCifri}__${signalNo}__`;
  document.title = `_Пров._по_писмо_${pismoNoBukviBG}-${pismoNoCifri}_и_сигнал_${signalNo}`;
...
// ============ Зануляване на всички използвани променливи ============
provVid1 = "";
provVid11 = "";
signalNoData = "";
pismoNoData = "";
